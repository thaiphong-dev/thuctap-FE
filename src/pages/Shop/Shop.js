...
export default function Shop() {
  ...
  const { filterProducts } = useSelector(
    (state) => state.ProductReducer || null
  );
  ...
  const filterProduct = () => {
    ...
    if (filterPrice === 1) {
      ...
      tempArr = tempArr.filter((item) => {
        return (
          item.discount > filterPrice - 100000 && item.discount < filterPrice
        );
      });
    }
  ...
  useEffect(() => {
    const filter = async () => {
      await ProductsApi.getProductType()
        .then((data) => {
          dispatch({
      ...
      // })
      let condition = {
        min: filterPrice - 100000 < 0 ? 0 : filterPrice - 100000,
        max: filterPrice,
        maTheLoai: filterCate,
      }
      ...
      setArrProduct(res.data);
        ...
        let data = res.data.filter(x => x.detail.filter(i => i.gia >= condition.min && i.gia <= condition.max).length > 0)
        setArrProduct(data);
      }
                      ...
                      <div className="product-item bg-light mb-4">
                        ...
                        <div className="text-center py-4">
                          ...
                          <div className="d-flex align-items-center justify-content-center mt-2">
                            {/* <h5>{addCommas(removeNonNumeric(item?.detail?.[0]?.gia))}</h5> */}
                ...
                {/* <div className="col-12">
                                    <nav>
                                        <ul className="pagination justify-content-center">
                                            ...
                                            {arrProduct?.map((item, index) => (
                                            <li key={index} className={index === 0 ? "page-item active" : "page-item" }><a className="page-link" href="#">{index + 1}</a></li>
                                            ))}
